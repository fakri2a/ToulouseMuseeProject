	...
	<body>
		...
		<div id="show-gestionnaire" class="content scaffold-show" role="main">
			...
			<ol class="property-list gestionnaire">
				...
				<g:if test="${gestionnaireInstance?.musee}">
				<li class="fieldcontain">
					<span id="musee-label" class="property-label"><g:message code="gestionnaire.musee.label" default="Musee" /></span>
					
						<span class="property-value" aria-labelledby="musee-label"><g:link controller="musee" action="show" id="${gestionnaireInstance?.musee?.id}">${gestionnaireInstance?.musee?.encodeAsHTML()}</g:link></span>
